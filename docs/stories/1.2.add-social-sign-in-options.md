...
### Data Models
- User model should accommodate social identity linking (Apple ID, Google ID) alongside email/SMS identities
- Social authentication tokens need temporary storage with same security patterns as JWT tokens [Source: docs/stories/1.1.implement-email-sms-sign-in.md#technical-constraints]
- Account reconciliation logic to prevent duplicate viewer accounts when same email used across social providers

### API Specifications
- Social authentication endpoints in Serverpod for Apple Sign-In token validation
- Google Sign-In token validation endpoint
- Account reconciliation endpoint to merge/link social identities with existing accounts
- Fallback authentication endpoint to switch to email OTP flow

### Component Specifications
- Social sign-in buttons following design system tokens from `design_system/` [Source: architecture/front-end-architecture.md#design-system]
- Sign-in flow orchestration using BLoC for complex flows and StatefulWidget for simple UI state [Source: architecture/front-end-architecture.md#state-management]
- Error handling for social auth failures with retry actions via shared `ErrorView` components [Source: architecture/front-end-architecture.md#error-handling]

### File Locations
- New social auth components under `features/auth/presentation/widgets/social_sign_in/`
- Auth providers under `features/auth/application/providers/`
- Social auth services under `features/auth/infrastructure/`
- Serverpod endpoints in `serverpod/packages/modules/auth/`
...
- Social authentication components align with documented feature-first structure under `features/auth/` [Source: architecture/coding-standards.md#flutter-client]
- No deviations identified from monorepo structure [Source: architecture/architecture.md#source-tree]

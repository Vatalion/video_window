...
- [ ] Implement OTP request UI and BLoC state to capture email or phone identifiers using shared design tokens (AC: 1) [Source: architecture/front-end-architecture.md#module-overview] [Source: architecture/front-end-architecture.md#state-management]
...
- [ ] Connect the OTP request flow to the Identity service `POST /auth/otp/request`, handling 200 and 429 responses with Twilio/SendGrid dispatch (AC: 1) [Source: architecture/architecture.md#identity-service] [Source: architecture/architecture.md#rest-api-spec-excerpt] [Source: architecture/architecture.md#twilio-verify-programmable-sms]
...
### Data Models
- The `users` table stores unique emails or optional phone numbers alongside roles and auth provider metadata, enabling OTP flows for viewers. [Source: architecture/architecture.md#database-schema-excerpt]
- Identity service modules rely on Postgres `users` and `sessions` tables to manage token issuance and refresh logic. [Source: architecture/architecture.md#identity-service]

### API Specifications
- `POST /auth/otp/request` accepts an `identifier` and `channel` (`email` or `sms`) and returns 200 on dispatch or 429 when the rate limit is exceeded. [Source: architecture/architecture.md#rest-api-spec-excerpt]
- OTP delivery leverages Twilio Verify/Programmable SMS with per-user rate enforcement; log message SIDs only. [Source: architecture/architecture.md#twilio-verify-programmable-sms]

### Component Specifications
- Flutter `auth` module owns OTP, session refresh, and logout UX; Identity service on Serverpod federates OTP and social login. [Source: architecture/story-component-mapping.md#epic-1--viewer-authentication--session-handling]
...
- Use shared analytics service injection to record auth events instead of duplicating event names. [Source: architecture/front-end-architecture.md#analytics-instrumentation]

### File Locations
- UI and state code for this story belong under `video_window/lib/features/auth/` following presentation, application, domain, and infrastructure layering. [Source: architecture/architecture.md#source-tree]
- Tests should mirror feature paths under `video_window/test/features/auth/` to match the project structure. [Source: architecture/architecture.md#testing-strategy]
...
- Authentication uses OTP for email/SMS with JWT/session tokens and requires enhanced refresh handling plus logout revocation with security monitoring. [Source: architecture/architecture.md#security] [Source: security/story-1.1-authentication-security-research.md]
...
- Twilio Verify enforces approximate 100 messages/sec throughput; implement client-side rate messaging aligned with backend protections. [Source: architecture/architecture.md#twilio-verify-programmable-sms]
...
- Planned changes align with the documented monorepo structure; no deviations identified. [Source: architecture/architecture.md#source-tree]
